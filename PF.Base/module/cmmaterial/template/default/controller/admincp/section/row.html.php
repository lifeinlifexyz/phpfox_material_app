...
?>
<h1 class="pull-left"><?php echo _p('Rows');?></h1><a class="btn btn-primary pull-right" href="{url link='admincp.cmmaterial.section.add.row'}">{phrase var='cmmaterial.create_one_now'}</a>.
<br>
<hr>
{if count($aSections)}
...
<form method="post" action="{url link='admincp.cmmaterial.section.row'}">
	<table id="js_drag_drop" cellpadding="0" cellspacing="0">
		<tr>
			...
			<th>{phrase var='cmmaterial.background'}</th>
			<th>{phrase var='cmmaterial.image_position'}</th>
		</tr>
		...
		<tr class="checkRow{if is_int($iKey/2)} tr{else}{/if}">
			...
			<td class="t_center">
				...
				<div class="link_menu">
					<ul>
						<li><a href="{url link='admincp.cmmaterial.section.add.row' id=$aItem.section_id}">{phrase var='cmmaterial.edit'}</a></li>
						<li><a href="{url link='admincp.cmmaterial.section.row' delete=$aItem.section_id}" onclick="return confirm('{phrase var='core.are_you_sure'}');">{phrase var='cmmaterial.delete'}</a></li>
					</ul>
				</div>		
			</td>	
			<td class="t_center">
			...
			<td>{$aItem.background}</td>
			<td>{$aItem.image_position}</td>
		</tr>
		{/foreach}
	</table>
